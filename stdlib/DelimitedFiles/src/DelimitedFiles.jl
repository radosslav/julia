...
mutable struct DLMOffsets <: DLMHandler
    ...
    function DLMOffsets(sbuff::String)
        offsets = Vector{Vector{Int}}(uninit, 1)
        offsets[1] = Vector{Int}(uninit, offs_chunk_size)
...
function store_cell(dlmoffsets::DLMOffsets, row::Int, col::Int,
    ...
    if length(offsets) < offidx
        ...
        offsets = Vector{Int}(uninit, offs_chunk_size)
        push!(oarr, offsets)
        offidx = 1
    end
    offsets[offidx] = row
...
function DLMStore(::Type{T}, dims::NTuple{2,Integer},
    ...
    DLMStore{T}(fill(SubString(sbuff,1,0), 1, ncols), Matrix{T}(uninit, nrows-hdr_offset, ncols),
