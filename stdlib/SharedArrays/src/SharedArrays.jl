...
mutable struct SharedArray{T,N} <: DenseArray{T,N}
    ...
    function SharedArray{T,N}(d,p,r,sn,s) where {T,N}
        S = new(RRID(),d,p,r,sn,s,0,view(Array{T}(uninit, ntuple(d->0,N)), 1:0))
...
function SharedArray{T,N}(dims::Dims{N}; init=false, pids=Int[]) where {T,N}
    ...
    local shm_seg_name = ""
    local s = Array{T}(uninit, ntuple(d->0,N))
    local S
    ...
    try
        ...
        refs = Vector{Future}(uninit, length(pids))
...
function finalize_refs(S::SharedArray{T,N}) where T where N
    if length(S.pids) > 0
        ...
        empty!(S.refs)
        init_loc_flds(S)
        S.s = Array{T}(uninit, ntuple(d->0,N))
        delete!(sa_refs, S.id)
    end
    S
end
...
function reshape(a::SharedArray{T}, dims::NTuple{N,Int}) where {T,N}
    ...
    refs = Vector{Future}(uninit, length(a.pids))
...
function init_loc_flds(S::SharedArray{T,N}, empty_local=false) where T where N
    if myid() in S.pids
        ...
        S.loc_subarr_1d = view(Array{T}(uninit, ntuple(d->0,N)), 1:0)
    end
end
