...
function tree_format(lilist::Vector{StackFrame}, counts::Vector{Int}, level::Int, cols::Int)
    ...
    strs = Vector{String}(uninit, length(lilist))
    showextra = false
    if level > nindent
...
function tree(io::IO, bt::Vector{Vector{UInt64}}, counts::Vector{Int},
    ...
    if fmt.combine
        ...
        # Generate counts
        dlen = length(d)
        lilist = Vector{StackFrame}(uninit, dlen)
        group = Vector{Vector{Int}}(uninit, dlen)
        n = Vector{Int}(uninit, dlen)
        i = 1
        for (key, v) in d
            lilist[i] = key
            group[i] = v
...
function liperm(lilist::Vector{StackFrame})
    comb = Vector{String}(uninit, length(lilist))
    for i = 1:length(lilist)
        li = lilist[i]
        if li != UNKNOWN
