...
function aupd_wrapper(T, matvecA!::Function, matvecB::Function, solveSI::Function, n::Integer,
    ...
    v     = Matrix{T}(uninit, n, ncv)
    workd = Vector{T}(uninit, 3*n)
    workl = Vector{T}(uninit, lworkl)
    rwork = cmplx ? Vector{TR}(uninit, ncv) : Vector{TR}()

    if isempty(v0)
        resid = Vector{T}(uninit, n)
        info  = Ref{BlasInt}(0)
    else
        resid = deepcopy(v0)
        info  = Ref{BlasInt}(1)
...
function eupd_wrapper(T, n::Integer, sym::Bool, cmplx::Bool, bmat::String,
    ...
    howmny = "A"
    select = Vector{BlasInt}(uninit, ncv)
    info   = Ref{BlasInt}(0)
    ...
    if cmplx
        d = Vector{T}(uninit, nev+1)
        sigmar = Ref{T}(sigma)
        workev = Vector{T}(uninit, 2ncv)
    ...
    elseif sym
        ...
        dr = Vector{T}(uninit, nev+1)
        di = Vector{T}(uninit, nev+1)
        fill!(dr,NaN)
        fill!(di,NaN)
        sigmar = Ref{T}(real(sigma))
        sigmai = Ref{T}(imag(sigma))
        workev = Vector{T}(uninit, 3*ncv)
        ...
        evec = complex.(Matrix{T}(uninit, n, nev+1), Matrix{T}(uninit, n, nev+1))
