...
mutable struct Graph
    ...
    function Graph(
        ...
        let spp = spp # Due to https://github.com/JuliaLang/julia/issues/15276
            ...
            extended_deps = [Vector{Dict{Int,BitVector}}(uninit, spp[p0]-1) for p0 = 1:np]::Vector{Vector{Dict{Int,BitVector}}}
        end
        for p0 = 1:np, v0 = 1:(spp[p0]-1)
            ...
            for (p1, vs) in req
                pv = pvers[p1]
                req_msk_p1 = BitArray(uninit, spp[p1] - 1)
    ...
    while !isempty(staged)
        ...
        for p0 in staged
            ...
            for (j1,p1) in enumerate(gadj[p0])
                ...
                added_constr1 = any!(BitVector(uninit, spp[p1]), sub_msk)
