...
let mt = MersenneTwister()
    ...
    @test issubset(rand!(mt, Vector{Int}(uninit, 100), 0:3:1000), 0:3:1000)
    ...
    rand(0:3:1000)
    rand!(Vector{Int}(uninit, 100), 0:3:1000)
    rand(coll)
    rand(coll, 2, 3)
end
...
ki = Vector{UInt64}(uninit, ziggurat_table_size)
wi = Vector{Float64}(uninit, ziggurat_table_size)
fi = Vector{Float64}(uninit, ziggurat_table_size)
# Tables for exponential variates
ke = Vector{UInt64}(uninit, ziggurat_table_size)
we = Vector{Float64}(uninit, ziggurat_table_size)
fe = Vector{Float64}(uninit, ziggurat_table_size)
...
let mt = MersenneTwister(0)
    ...
    for (i,T) in enumerate([Int8, UInt8, Int16, UInt16, Int32, UInt32, Int64, UInt64, Int128, Float16, Float32])
        A = Vector{T}(uninit, 16)
        B = Vector{T}(uninit, 31)
        rand!(mt, A)
        rand!(mt, B)
    ...
    srand(mt, 0)
    AF64 = Vector{Float64}(uninit, Random.dsfmt_get_min_array_size()-1)
...
let mt = MersenneTwister(0)
    ...
    b = Vector{Float64}(uninit, 1000) # should be 16-byte aligned
    c8 = Vector{UInt64}(uninit, 1001)
    pc8 = pointer(c8)
    if Int(pc8) % 16 == 0
...
for rng in ([], [MersenneTwister(0)], [RandomDevice()])
    ...
    for (C, T) in collections
        ...
        a5  = rand!(rng..., Array{T}(uninit, 5), C)                          ::Vector{T}
        a6  = rand!(rng..., Array{T}(uninit, 2, 3), C)                       ::Array{T, 2}
        a7  = rand!(rng..., GenericArray{T}(uninit, 5), C)                   ::GenericArray{T, 1}
        a8  = rand!(rng..., GenericArray{T}(uninit, 2, 3), C)                ::GenericArray{T, 2}
        a9  = rand!(rng..., OffsetArray(Array{T}(uninit, 5), 9), C)          ::OffsetArray{T, 1}
        a10 = rand!(rng..., OffsetArray(Array{T}(uninit, 2, 3), (-2, 4)), C) ::OffsetArray{T, 2}
        @test size(a1) == (5,)
    ...
    for f! in [rand!, randn!, randexp!]
        for T in functypes[f!]
            ...
            for A in (Vector{T}(uninit, 5),
                      Matrix{T}(uninit, 2, 3),
                      GenericArray{T}(uninit, 5),
                      GenericArray{T}(uninit, 2, 3),
                      OffsetArray(Array{T}(uninit, 5), -3),
                      OffsetArray(Array{T}(uninit, 2, 3), (4, 5)))
                local A
                f!(rng..., A)                    ::typeof(A)
                if f! === rand!
                    f!(rng..., A, X)             ::typeof(A)
    ...
    rand!(rng..., BitVector(uninit, 5))     ::BitArray{1}
    rand!(rng..., BitMatrix(uninit, 2, 3))  ::BitArray{2}
...
let mta = MersenneTwister(42), mtb = MersenneTwister(42)
    ...
    A, B = Vector{Int}(uninit, 10), Vector{Int}(uninit, 10)
