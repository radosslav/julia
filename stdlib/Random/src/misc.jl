...
```
"""
bitrand(r::AbstractRNG, dims::Dims)   = rand!(r, BitArray(uninit, dims))
bitrand(r::AbstractRNG, dims::Integer...) = rand!(r, BitArray(uninit, convert(Dims, dims)))

bitrand(dims::Dims)   = rand!(BitArray(uninit, dims))
bitrand(dims::Integer...) = rand!(BitArray(uninit, convert(Dims, dims)))
...
```
"""
randperm(r::AbstractRNG, n::Integer) = randperm!(r, Vector{Int}(uninit, n))
...
# Examples
```jldoctest
julia> randperm!(MersenneTwister(1234), Vector{Int}(uninit, 4))
4-element Array{Int64,1}:
 2
 1
 4
...
function randperm!(r::AbstractRNG, a::Array{<:Integer})
    ...
    @inbounds for i = 2:n
        j = 1 + rand(r, ltm52(i, mask))
        if i != j # a[i] is uninit (and could be #undef)
            a[i] = a[j]
        end
        a[j] = i
...
```
"""
randcycle(r::AbstractRNG, n::Integer) = randcycle!(r, Vector{Int}(uninit, n))
...
# Examples
```jldoctest
julia> randcycle!(MersenneTwister(1234), Vector{Int}(uninit, 6))
6-element Array{Int64,1}:
 3
 5
 4
