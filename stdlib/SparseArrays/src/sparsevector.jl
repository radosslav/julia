...
function sparsevec(dict::AbstractDict{Ti,Tv}) where {Tv,Ti<:Integer}
    m = length(dict)
    nzind = Vector{Ti}(uninit, m)
    nzval = Vector{Tv}(uninit, m)
...
function _dense2sparsevec(s::AbstractArray{Tv}, initcap::Ti) where {Tv,Ti}
    ...
    n = length(s)
    cap = initcap
    nzind = Vector{Ti}(uninit, cap)
    nzval = Vector{Tv}(uninit, cap)
    c = 0
    @inbounds for i = 1:n
        v = s[i]
        if v != 0
...
function _logical_index(A::SparseMatrixCSC{Tv}, I::AbstractArray{Bool}) where Tv
    ...
    colptrA = A.colptr; rowvalA = A.rowval; nzvalA = A.nzval
    rowvalB = Vector{Int}(uninit, nnzB)
    nzvalB = Vector{Tv}(uninit, nnzB)
    c = 1
    rowB = 1
...
function getindex(x::AbstractSparseVector{Tv,Ti}, I::AbstractUnitRange) where {Tv,Ti}
    ...
    jrgn = j0:j1
    mr = length(jrgn)
    rind = Vector{Ti}(uninit, mr)
    rval = Vector{Tv}(uninit, mr)
    if mr > 0
        c = 0
        for j in jrgn
            c += 1
...
function _absspvec_hcat(X::AbstractSparseVector{Tv,Ti}...) where {Tv,Ti}
    ...
    nzrow = Vector{Ti}(uninit, tnnz)
    nzval = Vector{Tv}(uninit, tnnz)
    roff = 1
    @inbounds for j = 1:n
        xj = X[j]
...
function _absspvec_vcat(X::AbstractSparseVector{Tv,Ti}...) where {Tv,Ti}
    ...
    # construction
    rnzind = Vector{Ti}(uninit, tnnz)
    rnzval = Vector{Tv}(uninit, tnnz)
    ir = 0
    len = 0
    @inbounds for j = 1:n
        xj = X[j]
...
function hvcat(rows::Tuple{Vararg{Int}}, X::_SparseConcatGroup...)
    ...
    tmp_rows = Vector{SparseMatrixCSC}(uninit, nbr)
    k = 0
...
macro unarymap_nz2z_z2z(op, TF)
    esc(quote
        function $(op)(x::AbstractSparseVector{Tv,Ti}) where Tv<:$(TF) where Ti<:Integer
            ...
            ynzind = Vector{Ti}(uninit, m)
            ynzval = Vector{R}(uninit, m)
            ir = 0
            @inbounds for j = 1:m
                i = xnzind[j]
                v = $(op)(xnzval[j])
...
function _binarymap(f::Function,
    ...
    rind = Vector{Int}(uninit, cap)
    rval = Vector{R}(uninit, cap)
    ir = 0
    ix = 1
    iy = 1
...
function _At_or_Ac_mul_B(tfun::Function, A::SparseMatrixCSC{TvA,TiA}, x::AbstractSparseVector{TvX,TiX}) where {TvA,TiA,TvX,TiX}
    ...
    ynzind = Vector{Ti}(uninit, n)
    ynzval = Vector{Tv}(uninit, n)
