...
function spmatmul(A::SparseMatrixCSC{Tv,Ti}, B::SparseMatrixCSC{Tv,Ti};
    ...
    colptrC = Vector{Ti}(uninit, nB+1)
    rowvalC = Vector{Ti}(uninit, nnzC)
    nzvalC = Vector{Tv}(uninit, nnzC)
...
function triu(S::SparseMatrixCSC{Tv,Ti}, k::Integer=0) where {Tv,Ti}
    ...
    rowval = Vector{Ti}(uninit, nnz)
    nzval = Vector{Tv}(uninit, nnz)
...
function sparse_diff1(S::SparseMatrixCSC{Tv,Ti}) where {Tv,Ti}
    ...
    rowval = Vector{Ti}(uninit, numnz)
    nzval = Vector{Tv}(uninit, numnz)
    numnz = 0
    colptr[1] = 1
    for col = 1 : n
        last_row = 0
...
function sparse_diff2(a::SparseMatrixCSC{Tv,Ti}) where {Tv,Ti}
    m,n = size(a)
    colptr = Vector{Ti}(uninit, max(n,1))
...
function normestinv(A::SparseMatrixCSC{T}, t::Integer = min(2,maximum(size(A)))) where T
    ...
    ind = Vector{Int64}(uninit, n)
    ind_hist = Vector{Int64}(uninit, maxiter * t)
...
function kron(x::SparseVector{Tv,Ti},y::SparseVector{Tv,Ti}) where {Tv,Ti}
    ...
    nzind = Vector{Ti}(uninit, nnzz) # the indices of nonzeros
    nzval = Vector{Tv}(uninit, nnzz) # the values of nonzeros
