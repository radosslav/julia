...
const umf_ctrl = Vector{Float64}(uninit, UMFPACK_CONTROL)
ccall((:umfpack_dl_defaults,:libumfpack), Cvoid, (Ptr{Float64},), umf_ctrl)
const umf_info = Vector{Float64}(uninit, UMFPACK_INFO)
...
for itype in UmfpackIndexTypes
    ...
    @eval begin
        ...
        function umf_extract(lu::UmfpackLU{Float64,$itype})
            ...
            Lp = Vector{$itype}(uninit, n_row + 1)
            Lj = Vector{$itype}(uninit, lnz) # L is returned in CSR (compressed sparse row) format
            Lx = Vector{Float64}(uninit, lnz)
            Up = Vector{$itype}(uninit, n_col + 1)
            Ui = Vector{$itype}(uninit, unz)
            Ux = Vector{Float64}(uninit, unz)
            P  = Vector{$itype}(uninit, n_row)
            Q  = Vector{$itype}(uninit, n_col)
            Rs = Vector{Float64}(uninit, n_row)
        ...
        function umf_extract(lu::UmfpackLU{ComplexF64,$itype})
            ...
            Lj = Vector{$itype}(uninit, lnz) # L is returned in CSR (compressed sparse row) format
            Lx = Vector{Float64}(uninit, lnz)
            Lz = Vector{Float64}(uninit, lnz)
            Up = Vector{$itype}(uninit, n_col + 1)
            Ui = Vector{$itype}(uninit, unz)
            Ux = Vector{Float64}(uninit, unz)
            Uz = Vector{Float64}(uninit, unz)
            P  = Vector{$itype}(uninit, n_row)
            Q  = Vector{$itype}(uninit, n_col)
            Rs = Vector{Float64}(uninit, n_row)
