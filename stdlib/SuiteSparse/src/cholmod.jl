...
const cholmod_com_offsets = Vector{Csize_t}(uninit, 19)
...
const build_version_array = Vector{Cint}(uninit, 3)
...
function __init__()
    try
        ...
        if Libdl.dlsym_e(Libdl.dlopen("libcholmod"), :cholmod_version) != C_NULL
            current_version_array = Vector{Cint}(uninit, 3)
...
function Matrix{T}(D::Dense{T}) where T
    s = unsafe_load(pointer(D))
    a = Matrix{T}(uninit, s.nrow, s.ncol)
    copyto!(a, D)
end
...
function Vector{T}(D::Dense{T}) where T
    ...
    copyto!(Vector{T}(uninit, size(D, 1)), D)
end
...
function sparse(F::Factor)
    ...
    p = get_perm(F)
    if p != [1:s.n;]
        pinv = Vector{Int}(uninit, length(p))
        for k = 1:length(p)
            pinv[p[k]] = k
        end
        A = A[pinv,pinv]
...
function getLd!(S::SparseMatrixCSC)
    d = Vector{eltype(S)}(uninit, size(S, 1))
    fill!(d, 0)
    col = 1
    for k = 1:nnz(S)
