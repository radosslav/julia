...
mutable struct Interface
    ...
    function Interface(reqs::Requires, deps::Dict{String,Dict{VersionNumber,Available}})
        ...
        ## generate wveights:
        vweight = Vector{Vector{VersionWeight}}(uninit, np)
        for p0 = 1:np
            pvers0 = pvers[p0]
            spp0 = spp[p0]
            vweight0 = vweight[p0] = Vector{VersionWeight}(uninit, spp0)
            for v0 = 1:spp0-1
...
function enforce_optimality!(sol::Vector{Int}, interface::Interface)
    ...
    pdeps = [Vector{Requires}(uninit, spp[p0]-1) for p0 = 1:np]
