...
function RowVector{T}(::Uninitialized, n::Int) where {T}
    Base.depwarn(_RowVector_depstring(), :RowVector)
    return RowVector{T}(Vector{transpose_type(T)}(uninit, n))
end
function RowVector{T}(::Uninitialized, n1::Int, n2::Int) where {T}
    Base.depwarn(_RowVector_depstring(), :RowVector)
    return n1 == 1 ? RowVector{T}(Vector{transpose_type(T)}(uninit, n2)) :
        error("RowVector expects 1×N size, got ($n1,$n2)")
end
function RowVector{T}(::Uninitialized, n::Tuple{Int}) where {T}
    Base.depwarn(_RowVector_depstring(), :RowVector)
    return RowVector{T}(Vector{transpose_type(T)}(uninit, n[1]))
end
function RowVector{T}(::Uninitialized, n::Tuple{Int,Int}) where {T}
    Base.depwarn(_RowVector_depstring(), :RowVector)
    return n[1] == 1 ? RowVector{T}(Vector{transpose_type(T)}(uninit, n[2])) :
...
# deprecate RowVector{T}(shape...) constructors to RowVector{T}(uninit, shape...) equivalents
@deprecate RowVector{T}(n::Int) where {T}               RowVector{T}(uninit, n)
@deprecate RowVector{T}(n1::Int, n2::Int) where {T}     RowVector{T}(uninit, n1, n2)
@deprecate RowVector{T}(n::Tuple{Int}) where {T}        RowVector{T}(uninit, n)
@deprecate RowVector{T}(n::Tuple{Int,Int}) where {T}    RowVector{T}(uninit, n)
