...
for (gels, gesv, getrs, getri, elty) in
    ...
    @eval begin
        ...
        function gels!(trans::Char, A::AbstractMatrix{$elty}, B::AbstractVecOrMat{$elty})
            ...
            ssr = Vector{$elty}(uninit, size(B, 2))
            for i = 1:size(B,2)
                x = zero($elty)
                for j = k+1:size(B,1)
                    x += abs2(B[j,i])
...
for (gesvx, elty) in
    ...
    @eval begin
        ...
        function gesvx!(fact::Char, trans::Char, A::AbstractMatrix{$elty},
            ...
            work  = Vector{$elty}(uninit, 4n)
            iwork = Vector{BlasInt}(uninit, n)
            info  = Ref{BlasInt}()
...
for (geev, gesvd, gesdd, ggsvd, elty, relty) in
    ...
    @eval begin
        ...
        function gesdd!(job::Char, A::AbstractMatrix{$elty})
            ...
            cmplx  = eltype(A)<:Complex
            if cmplx
                rwork = Vector{$relty}(uninit, job == 'N' ? 7*minmn : minmn*max(5*minmn+7, 2*max(m,n)+2*minmn+1))
            end
            iwork  = Vector{BlasInt}(uninit, 8*minmn)
            info   = Ref{BlasInt}()
            for i = 1:2  # first call returns lwork as work[1]
                if cmplx
        ...
        function gesvd!(jobu::Char, jobvt::Char, A::AbstractMatrix{$elty})
            ...
            cmplx  = eltype(A) <: Complex
            if cmplx
                rwork = Vector{$relty}(uninit, 5minmn)
            end
            lwork  = BlasInt(-1)
            info   = Ref{BlasInt}()
        ...
        function ggsvd!(jobu::Char, jobv::Char, jobq::Char, A::AbstractMatrix{$elty}, B::AbstractMatrix{$elty})
            ...
            p = size(B, 1)
            k = Vector{BlasInt}(uninit, 1)
            l = Vector{BlasInt}(uninit, 1)
            lda = max(1,stride(A, 2))
            ...
            work = Vector{$elty}(uninit, max(3n, m, p) + n)
            cmplx = eltype(A) <: Complex
            if cmplx
                rwork = Vector{$relty}(uninit, 2n)
            end
...
for (geevx, ggev, elty) in
    ...
    @eval begin
        ...
        function geevx!(balanc::Char, jobvl::Char, jobvr::Char, sense::Char, A::AbstractMatrix{$elty})
            ...
            iwork = Vector{BlasInt}(uninit, iworksize)
            info = Ref{BlasInt}()
            for i = 1:2  # first call returns lwork as work[1]
...
for (laic1, elty) in
    ...
    @eval begin
        ...
        function laic1!(job::Integer, x::AbstractVector{$elty},
            ...
            sestpr = Vector{$elty}(uninit, 1)
            s = Vector{$elty}(uninit, 1)
            c = Vector{$elty}(uninit, 1)
...
for (posv, potrf, potri, potrs, pstrf, elty, rtyp) in
    ...
    @eval begin
        ...
        function pstrf!(uplo::Char, A::AbstractMatrix{$elty}, tol::Real)
            ...
            rank = Vector{BlasInt}(uninit, 1)
            work = Vector{$rtyp}(uninit, 2n)
            info = Ref{BlasInt}()
...
#     , (:cstev_,:ComplexF32)
    ...
    @eval begin
        function stev!(job::Char, dv::AbstractVector{$elty}, ev::AbstractVector{$elty})
            ...
            work = Vector{$elty}(uninit, max(1, 2n-2))
            info = Ref{BlasInt}()
        ...
        function stebz!(range::Char, order::Char, vl::$elty, vu::$elty, il::Integer, iu::Integer, abstol::Real, dv::AbstractVector{$elty}, ev::AbstractVector{$elty})
            ...
            m = Ref{BlasInt}()
            nsplit = Vector{BlasInt}(uninit, 1)
            w = similar(dv, $elty, n)
            tmp = 0.0
            iblock = similar(dv, BlasInt,n)
            isplit = similar(dv, BlasInt,n)
            work = Vector{$elty}(uninit, 4*n)
            iwork = Vector{BlasInt}(uninit, 3*n)
            info = Ref{BlasInt}()
        ...
        function stein!(dv::AbstractVector{$elty}, ev_in::AbstractVector{$elty}, w_in::AbstractVector{$elty}, iblock_in::AbstractVector{BlasInt}, isplit_in::AbstractVector{BlasInt})
            ...
            work  = Vector{$elty}(uninit, 5*n)
            iwork = Vector{BlasInt}(uninit, n)
            ifail = Vector{BlasInt}(uninit, m)
            info  = Ref{BlasInt}()
...
for (sysv, sytrf, sytri, sytrs, syconvf, elty) in
    ...
    @eval begin
        ...
        function syconvf_rook!(uplo::Char, way::Char,
                                ...
                                e::AbstractVector{$elty} = Vector{$elty}(uninit, length(ipiv)))
            # extract
            n = checksquare(A)
            lda = max(1, stride(A, 2))
...
for (syev, syevr, sygvd, elty, relty) in
    ...
    @eval begin
        ...
        function syev!(jobz::Char, uplo::Char, A::AbstractMatrix{$elty})
            ...
            work  = Vector{$elty}(uninit, 1)
            lwork = BlasInt(-1)
            rwork = Vector{$relty}(uninit, max(1, 3n-2))
            info  = Ref{BlasInt}()
            for i = 1:2  # first call returns lwork as work[1]
                ccall((@blasfunc($syev), liblapack), Cvoid,
                      (Ref{UInt8}, Ref{UInt8}, Ref{BlasInt}, Ptr{$elty}, Ref{BlasInt},
...
for (bdsdc, elty) in
    ...
    @eval begin
        ...
        function bdsdc!(uplo::Char, compq::Char, d::AbstractVector{$elty}, e_::AbstractVector{$elty})
            ...
            work  = Vector{$elty}(uninit, lwork)
            iwork = Vector{BlasInt}(uninit, 8n)
            info  = Ref{BlasInt}()
