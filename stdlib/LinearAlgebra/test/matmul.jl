...
@testset "matrices with zero dimensions" begin
    for (dimsA, dimsB, dimsC) in (
            ...
            ((3,0), (0,0), (3,0)),
            ((0,0), (0,0), (0,0)) )
        @test Matrix{Float64}(uninit, dimsA) * Matrix{Float64}(uninit, dimsB) == zeros(dimsC)
    end
    @test Matrix{Float64}(uninit, 5, 0) |> t -> t't == zeros(0,0)
    @test Matrix{Float64}(uninit, 5, 0) |> t -> t*t' == zeros(5,5)
    @test Matrix{ComplexF64}(uninit, 5, 0) |> t -> t't == zeros(0,0)
    @test Matrix{ComplexF64}(uninit, 5, 0) |> t -> t*t' == zeros(5,5)
end
@testset "2x2 matmul" begin
    ...
    @test_throws DimensionMismatch mul!(Matrix{Float64}(uninit,3,3), AA, BB)
end
@testset "3x3 matmul" begin
    ...
    @test_throws DimensionMismatch mul!(Matrix{Float64}(uninit,4,4), AA, BB)
end
...
@testset "Generic integer matrix multiplication" begin
    ...
    AA = rand(1:20, 5, 5) .- 10
    BB = rand(1:20, 5, 5) .- 10
    CC = Matrix{Int}(uninit, size(AA, 1), size(BB, 2))
...
@testset "syrk & herk" begin
    ...
    A5x5, A6x5 = Matrix{Float64}.(uninit, ((5, 5), (6, 5)))
...
@testset "mul! (scaling)" begin
    A5x5, b5, C5x6 = Array{Float64}.(uninit,((5,5), 5, (5,6)))
...
@testset "gemv! and gemm_wrapper for $elty" for elty in [Float32,Float64,ComplexF64,ComplexF32]
    ...
    @test LinearAlgebra.gemv!(elty[], 'N', Matrix{elty}(uninit,0,0), elty[]) == elty[]
    @test LinearAlgebra.gemv!(x10, 'N', Matrix{elty}(uninit,10,0), elty[]) == zeros(elty,10)
