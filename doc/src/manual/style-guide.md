...
```julia
a = Vector{Union{Int,AbstractString,Tuple,Array}}(uninit, n)
```

In this case `Vector{Any}(uninit, n)` is better. It is also more helpful to the compiler to annotate specific
...
9. **Varargs**.
   ...
   For example, in `Matrix{T}(uninit, dims)`, the dimensions can be given as a
   [`Tuple`](@ref), e.g. `Matrix{T}(uninit, (1,2))`, or as [`Vararg`](@ref)s,
   e.g. `Matrix{T}(uninit, 1, 2)`.
