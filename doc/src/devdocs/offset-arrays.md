...
Storage is often allocated with `Array{Int}(uninit, dims)` or `similar(A, args...)`. When the result needs
...
that matches `A`.  (The most obvious implementation is to allocate an `Array{Int}(uninit, size(A))` and
