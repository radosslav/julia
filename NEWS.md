...
----------------
  ...
  * Uninitialized `BitArray` constructors of the form `BitArray[{N}](shape...)` have been
    ...
    `BitArray[{N}](uninit, shape...)`. For example, `BitVector(3)` is now
    `BitVector(uninit, 3)`, `BitMatrix((2, 4))` is now
    `BitMatrix(uninit, (2, 4))`, and `BitArray{3}(11, 13, 17)` is now
    `BitArray{3}(uninit, 11, 14, 17)` ([#24785]).
...
---------------------
  ...
  * Uninitialized `Array` constructors of the form
    ...
    as in `Array[{T,N}](uninit, shape...)`. For example,
    `Vector(3)` is now `Vector(uninit, 3)`, `Matrix{Int}((2, 4))` is now,
    `Matrix{Int}(uninit, (2, 4))`, and `Array{Float32,3}(11, 13, 17)` is now
    `Array{Float32,3}(uninit, 11, 13, 17)` ([#24781]).
  ...
  * Uninitialized `RowVector` constructors of the form `RowVector{T}(shape...)` have been
    deprecated in favor of equivalents accepting `uninit` (an alias for
    `Uninitialized()`) as their first argument, as in
    `RowVector{T}(uninit, shape...)`. For example, `RowVector{Int}(3)` is now
    `RowVector{Int}(uninit, 3)`, and `RowVector{Float32}((1, 4))` is now
    `RowVector{Float32}(uninit, (1, 4))` ([#24786]).
  ...
  * `read(io, type, dims)` is deprecated to `read!(io, Array{type}(uninit, dims))` ([#21450]).
