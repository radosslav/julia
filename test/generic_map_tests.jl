...
function run_map_equivalence_tests(mapf)
    ...
    testmap_equivalence(mapf, x->x ? false : true, BitMatrix(uninit, 10,10))
    testmap_equivalence(mapf, x->"foobar", BitMatrix(uninit, 10,10))
    testmap_equivalence(mapf, (x,y,z)->string(x,y,z), BitVector(uninit, 10), fill(1.0, 10), "1234567890")
end
