...
function test_primitives(::Type{T}, shape, ::Type{TestAbstractArray}) where T
    ...
    @test_throws BoundsError copyto!(Vector{Int}(uninit, 10), [1:11...])
...
function test_cat(::Type{TestAbstractArray})
    ...
    b2hcat = Array{Float64}(uninit, 3, 6, 3)
    b1 = reshape([1:9...], 3, 3)
    ...
    b2hcat[:, :, 2] = hcat(b2, b2)
    b2hcat[:, :, 3] = hcat(b3, b3)
    b3hcat = Array{Float64}(uninit, 3, 9, 3)
