...
a_global_closure_vector = Vector{Any}(uninit, 3)
for i = 1:3
    let ii = i
...
let
    local a
    a = Vector{Any}(uninit, 2)
    @test !isassigned(a,1) && !isassigned(a,2)
    a[1] = 1
    @test isassigned(a,1) && !isassigned(a,2)
    a = Vector{Float64}(uninit,1)
    @test isassigned(a,1)
    @test isassigned(a)
    @test !isassigned(a,2)
end
...
let
    ...
    # issue #1886
    X = [1:4;]
    r = Vector{UnitRange{Int}}(uninit, 1)
    r[1] = 2:3
    X[r...] *= 2
    @test X == [1,4,6,4]
end
...
@test repr(Vector{Foo2517}(uninit, 1)) == "$(curmod_prefix)Foo2517[$(curmod_prefix)Foo2517()]"
...
let
    function foo(x)
        ret=Vector{typeof(x[1])}(uninit, length(x))
        for j = 1:length(x)
            ret[j] = x[j]
        end
        return ret
    end
    x = Vector{Union{Dict{Int64,AbstractString},Array{Int64,3},Number,AbstractString,Nothing}}(uninit, 3)
    x[1] = 1.0
    x[2] = 2.0
    x[3] = 3.0
...
# issue #5150
f5150(T) = Vector{Rational{T}}(uninit, 1)
...
function test5884()
    star = Vector{Polygon5884}(uninit, (3,))
...
# issue #6784
@test ndims(Array{Array{Float64}}(uninit, 3,5)) == 2
@test ndims(Array{Array}(uninit, 3,5)) == 2
...
# issue #11772
@test_throws UndefRefError (Vector{Any}(uninit, 5)...,)
...
let x = Vector{Empty12394}(uninit, 1), y = [Empty12394()]
...
@test read!(IO13433(), Array{UInt8}(uninit, 4)) == [0x01, 0x01, 0x01, 0x01]
...
@test Int(pointer(Vector{Float64}(uninit, 1024))) % 64 == 0
...
let
    function arrayset_unknown_dim(::Type{T}, n) where T
        Base.arrayset(true, reshape(Vector{T}(uninit, 1), fill(1, n)...), 2, 1)
    end
...
# Copy of `#undef`
copyto!(Vector{Any}(uninit, 10), Vector{Any}(uninit, 10))
...
let a = Vector{Core.TypeofBottom}(uninit, 2)
    @test a[1] == Union{}
    @test a == [Union{}, Union{}]
end
...
let ni128 = sizeof(FP128test) ÷ sizeof(Int),
    ...
    nbit = sizeof(Int) * 8,
    arr = Vector{FP128align}(uninit, 2),
...
for U in boxedunions
    local U
    for N in (1, 2, 3, 4)
        A = Array{U}(uninit, ntuple(x->0, N)...)
        @test isempty(A)
        @test sizeof(A) == 0

        A = Array{U}(uninit, ntuple(x->10, N)...)
        @test length(A) == 10^N
...
# copyto!
A23567 = Vector{Union{Float64, Nothing}}(uninit, 5)
