...
@deprecate read(s::IO, t::Type, d1::Int, dims::Int...) read!(s, Array{t}(uninit, tuple(d1,dims...)))
@deprecate read(s::IO, t::Type, d1::Integer, dims::Integer...) read!(s, Array{t}(uninit, convert(Tuple{Vararg{Int}},tuple(d1,dims...))))
@deprecate read(s::IO, t::Type, dims::Dims) read!(s, Array{t}(uninit, dims))
...
# deprecate BitArray{...}(shape...) constructors to BitArray{...}(uninit, shape...) equivalents
@deprecate BitArray{N}(dims::Vararg{Int,N}) where {N}   BitArray{N}(uninit, dims)
@deprecate BitArray(dims::NTuple{N,Int}) where {N}      BitArray(uninit, dims...)
@deprecate BitArray(dims::Integer...)                   BitArray(uninit, dims)
...
# deprecate Array(shape...)-like constructors to Array(uninit, shape...) equivalents
# --> former primitive constructors
@deprecate Array{T,1}(m::Int) where {T}                      Array{T,1}(uninit, m)
@deprecate Array{T,2}(m::Int, n::Int) where {T}              Array{T,2}(uninit, m, n)
@deprecate Array{T,3}(m::Int, n::Int, o::Int) where {T}      Array{T,3}(uninit, m, n, o)
@deprecate Array{T,N}(d::Vararg{Int,N}) where {T,N}          Array{T,N}(uninit, d)
@deprecate Array{T,N}(d::NTuple{N,Int}) where {T,N}          Array{T,N}(uninit, d)
@deprecate Array{T}(m::Int) where {T}                        Array{T}(uninit, m)
@deprecate Array{T}(m::Int, n::Int) where {T}                Array{T}(uninit, m, n)
@deprecate Array{T}(m::Int, n::Int, o::Int) where {T}        Array{T}(uninit, m, n, o)
@deprecate Array{T}(d::NTuple{N,Int}) where {T,N}            Array{T}(uninit, d)
# --> former convenience constructors
@deprecate Vector{T}(m::Integer) where {T}                          Vector{T}(uninit, m)
@deprecate Matrix{T}(m::Integer, n::Integer) where {T}              Matrix{T}(uninit, m, n)
@deprecate Array{T}(m::Integer) where {T}                           Array{T}(uninit, m)
@deprecate Array{T}(m::Integer, n::Integer) where {T}               Array{T}(uninit, m, n)
@deprecate Array{T}(m::Integer, n::Integer, o::Integer) where {T}   Array{T}(uninit, m, n, o)
@deprecate Array{T}(d::Integer...) where {T}                        Array{T}(uninit, d)
@deprecate Vector(m::Integer)                                       Vector(uninit, m)
@deprecate Matrix(m::Integer, n::Integer)                           Matrix(uninit, m, n)
...
@deprecate_binding uninitialized uninit
