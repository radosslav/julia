...
function show(io::IO, ::MIME"text/plain", t::AbstractDict{K,V}) where {K,V}
    ...
    if limit
        ...
        ks = Vector{AbstractString}(uninit, min(rows, length(t)))
        vs = Vector{AbstractString}(uninit, min(rows, length(t)))
        keylen = 0
        vallen = 0
...
function sourceinfo_slotnames(src::CodeInfo)
    ...
    printnames = Vector{String}(uninit, length(slotnames))
...
function show_unquoted(io::IO, ex::Expr, indent::Int, prec::Int)
    ...
    elseif head === :macrocall && nargs >= 2
        ...
        if prec >= 0
            ...
            show_args = Vector{Any}(uninit, length(args) - 1)
            show_args[1] = args[1]
