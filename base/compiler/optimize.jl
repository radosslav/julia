...
const ENABLE_VERIFY_VALUEINFO = (tmp = Array{Bool,0}(uninit); tmp[] = false; tmp)
...
function substitute!(
    ...
    if isa(e, PhiNode)
        values = Vector{Any}(uninit, length(e.values))
...
function ssavalue_increment(body::PhiNode, incr)
    values = Vector{Any}(uninit, length(body.values))
    for i = 1:length(values)
...
=#
    for ninline = 1:100
        ata = Vector{Any}(uninit, length(e.args))
        ata[1] = ft
        ...
        if f === _apply
            na = length(e.args)
            newargs = Vector{Any}(uninit, na-2)
            newstmts = Any[]
            effect_free_upto = 0
            for i = 3:na
                aarg = e.args[i]
...
function split_disjoint_assign!(ctx::AllocOptContext, info, key)
    ...
    deftypes = Vector{Any}(uninit, ndefs)
    for i in 1:ndefs
        def = info.defs[i]
        defex = (def.assign::Expr).args[2]
...
function structinfo_constant(ctx::AllocOptContext, @nospecialize(v), vt::DataType)
    nf = fieldcount(vt)
    if vt <: Tuple
        si = StructInfo(Vector{Any}(uninit, nf), Symbol[], vt, false, false)
    else
        si = StructInfo(Vector{Any}(uninit, nf), collect(Symbol, fieldnames(vt)),
                        vt, false, false)
    end
    for i in 1:nf
        if isdefined(v, i)
...
function split_struct_alloc_single!(ctx::AllocOptContext, info, key, nf, has_preserve,
    ...
    if !isempty(ctx.undef_fld) && has_setfield_undef
        flag_vars = Vector{SlotNumber}(uninit, nf)
    end
    vars = Vector{Any}(uninit, nf)
