...
default is an `Array{element_type}(uninit, dims...)`.
...
similar(a::AbstractArray, ::Type{T}, dims::Dims{N}) where {T,N}    = Array{T,N}(uninit, dims)
...
`Array{Int}(uninit, size(A))`, but if `A` has unconventional indexing then the
...
vcat(X::Number...) = hvcat_fill(Vector{promote_typeof(X...)}(uninit, length(X)), X)
hcat(X::Number...) = hvcat_fill(Matrix{promote_typeof(X...)}(uninit, 1,length(X)), X)
typed_vcat(::Type{T}, X::Number...) where {T} = hvcat_fill(Vector{T}(uninit, length(X)), X)
typed_hcat(::Type{T}, X::Number...) where {T} = hvcat_fill(Matrix{T}(uninit, 1,length(X)), X)
...
cat_similar(A, T, shape) = Array{T}(uninit, shape)
...
function _cat(A, shape::NTuple{N}, catdims, X...) where N
    offsets = zeros(Int, N)
    inds = Vector{UnitRange{Int}}(uninit, N)
