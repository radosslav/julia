...
julia> a = [1, 1, 1]; A = fill!(Vector{Vector{Int}}(uninit, 3), a); a[1] = 2; A
3-element Array{Array{Int64,1},1}:
 [2, 1, 1]
 [2, 1, 1]
 [2, 1, 1]

julia> x = 0; f() = (global x += 1; x); fill!(Vector{Int}(uninit, 3), f())
3-element Array{Int64,1}:
 1
 1
 1
...
julia> dest = OffsetArray{Int}(uninit, (0:3,2:5))
...
function extrema(A::AbstractArray, dims)
    sz = [size(A)...]
    sz[[dims...]] = 1
    B = Array{Tuple{eltype(A),eltype(A)}}(uninit, sz...)
    return extrema!(B, A)
end
