...
function getpass(prompt::AbstractString)
    print(prompt)
    flush(stdout)
    p = Vector{UInt8}(uninit, 128) # mimic Unix getpass in ignoring more than 128-char passwords
...
if Sys.iswindows()
    ...
    function winprompt(message, caption, default_username; prompt_username = true)
        ...
        credbuf = Vector{UInt8}(uninit, 1024)
...
function _crc32c(io::IO, nb::Integer, crc::UInt32=0x00000000)
    ...
    buf = Vector{UInt8}(uninit, min(nb, 24576))
