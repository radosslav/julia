...
Construct an uninitialized [`Vector{T}`](@ref) of length `n`. See [`uninit`](@ref).

# Examples
```julia-repl
julia> Vector{Float64}(uninit, 3)
...
Construct an uninitialized [`Matrix{T}`](@ref) of size `m`×`n`. See [`uninit`](@ref).

# Examples
```julia-repl
julia> Matrix{Float64}(uninit, 2, 3)
2×3 Array{Float64,2}:
...
# Examples
```julia-repl
julia> A = Array{Float64,2}(uninit, 2, 3) # N given explicitly
2×3 Array{Float64,2}:
 6.90198e-310  6.90198e-310  6.90198e-310
 6.90198e-310  6.90198e-310  0.0

julia> B = Array{Float64}(uninit, 2) # N determined by the input
...
would like an uninitialized array. See also [`uninit`](@ref),
...
# Examples
```julia-repl
julia> Array{Float64,1}(uninit, 3)
3-element Array{Float64,1}:
 2.2752528595e-314
 2.202942107e-314
 2.275252907e-314
